...
//	}
	...
	static private Bitmap getBitmapFromDrawable(Drawable drawable, int xRes, int yRes) {
		final Bitmap bmp = Bitmap.createBitmap(xRes, yRes, Bitmap.Config.ARGB_8888);
			...
			if (icons) {
				...
				if (!cacheValid || !iconFile.exists()) {
					...
					try {
						...
						Bitmap bmp = getBitmapFromDrawable(d, iconRes, iconRes);
						FileOutputStream out = new FileOutputStream(iconFile);
						bmp.compress(CompressFormat.PNG, 100, out);
						out.close();
					} catch (Exception e) {
						Log.e("AppStart", "icon "+e);
						iconFile.delete();
					}
				}
			}
